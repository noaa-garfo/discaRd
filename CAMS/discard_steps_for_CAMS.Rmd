...
bdat_focal <- bdat %>% 
	filter(YEAR == 2019) %>% 
	# mutate(REGION = ifelse(AREA < 600, 'N', 'S')) %>% 
...
bdat_focal = bdat %>% 
  ...
  dplyr::summarise(KALL = sum(max(OBS_HAUL_KALL_TRIP, na.rm = T)*max(PRORATE)), BYCATCH = sum(SPECIES_DISCARD, na.rm = T)) %>% 	              mutate(KALL = replace_na(KALL, 0)
				        , BYCATCH = replace_na(BYCATCH, 0)) %>% 
	ungroup()
...
```{r get trip data from oracle table}
#---------------------------------------------------------------------------------------#
...
bdat_focal <- bdat %>% 
	filter(YEAR == year) %>% 
	mutate(STRATA = paste(GEARTYPE, MESHGROUP, AREA, sep = '_')) %>%
...
run_discard <- function(obstab, ddat_focal, year = 2019, species_nespp3 = '802'){ #, strata = paste(GEARTYPE, MESHGROUP, AREA, sep = '_')
