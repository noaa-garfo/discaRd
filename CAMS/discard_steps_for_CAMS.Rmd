...
assumed_discard = bdat %>% 
	group_by(OBS_GEAR, OBS_MESHGROUP) %>% 
	summarise(KALL = sum(CAMS_KALL), DISCARD = sum(DISCARD), dk = sum(DISCARD, na.rm = T)/sum(CAMS_KALL, na.rm = T))
	
# set up bdat for discaRd
bdat_focal <- bdat %>% 
	filter(YEAR == 2019) %>% 
	mutate(STRATA = paste(OBS_GEAR, OBS_MESHGROUP, sep = '_')) %>% 
	# group_by(YEAR, OBS_GEAR, OBS_MESHGROUP, AREA) %>% 
	mutate(KALL = sum(CAMS_KALL), BYCATCH = sum(DISCARD))
...
ddat_focal <- dat %>% 
		...
		summarise(LIVE_POUNDS = sum(POUNDS, na.rm = T)) %>% 
		collect()
