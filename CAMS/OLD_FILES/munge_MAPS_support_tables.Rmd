...
for (i in 1:nrow(t_areas)){
  ...
  for (j in 1:length(definitions)){
    species_estimation_region <- as.character(t_areas[i,definitions[j]])
...
#stat_df$species_estimation_region[which(is.na(stat_df$species_estimation_region))] <- "UNIT"
...
p[[defn]] <- ggplot(merged) + geom_sf(aes(fill=factor(species_estimation_region))) + theme_minimal() +
