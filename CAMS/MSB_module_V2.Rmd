...
## This module is for a single species. It's posible that species with identical CAMS_GEAR_GROUP groupings can be done together. The assigment of this is done in the first step using SQL.  
...
species_nespp3 = '802'  # Illex squid
...
```{r set up disacRd elements for Illex}

# setup stratification variables
stratvars = c('CAMS_GEAR_GROUP', 'MESHGROUP', 'HALFOFYEAR', 'REGION')
...
# park the final rate into the trips table
 trans_rate_df %>% 
   ...
   summarise(inseason_rate_d = sum(in_season_rate*SUBTRIP_KALL, na.rm = T)
             , trans_rate_d = sum(trans_rate*SUBTRIP_KALL, na.rm = T)
             , final_rate_d = sum(final_rate*SUBTRIP_KALL, na.rm = T))
 
```

```{r compare to ACL accounting for 2019}
...
acl_tab = readr::read_csv('summary_table.csv') %>% 
	mutate(SPPNM = c("BUTTERFISH",  "ATLANTIC MACKEREL" ,  "LONGFIN SQUID"  ,   "ILLEX SQUID"))
...
out_res = d_focal$res %>% 
	dplyr::select(DMIS_TRIP_ID, VTRSERNO, NESPP3 , DISCARD)
