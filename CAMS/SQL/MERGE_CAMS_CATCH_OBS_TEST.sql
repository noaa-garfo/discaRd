...
--        AND OBS_LINK1 in o.link1       
        AND extract(year from a.obs_land) > 2016
    )
...
--    , o.obs_link1
--    , o.min_link1
--    , count(distinct(vtrserno)) over(partition by obs_link1) as nvtr_link1 -- count how many vtrs for each link1
--    , count(distinct(area)) over(partition by obs_link1) as narea_link1 -- count how many CAMS areas for each link1
--    from MAPS.CAMS_LANDINGS d
--    left join (  --adds observer link field
--         select * 
--         from vtr_link
...
CREATE INDEX yearidx_test ON CAMS_OBS_CATCH_TEST(YEAR, ITIS_TSN, LINK1, LINK3)
