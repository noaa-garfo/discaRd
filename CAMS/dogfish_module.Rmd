...
save(ddat_focal,file="ddat_focal_dogfishFY18.Rdata")
save(ddat_prev,file="ddat_prev_dogfishFY18.Rdata")
...
```{r load ddats, eval = T}
load(file="ddat_focal_dogfishFY18.Rdata")
load(file="ddat_prev_dogfishFY18.Rdata")
```
...
gf_ex$res$Discard_Mortality_Ratio[is.na(gf_ex$res$Discard_Mortality_Ratio)] <- 1
...
bdat_mod <- bdat_gf %>% filter(
  ...
  mutate(STRATA = gsub("-","_",STRATA))


mod <- glm(DISCARD~offset(log(OBS_KALL)) + CAMS_GEAR_GROUP + MESHGROUP + HALFOFYEAR + REGION,
           family=poisson(),data=bdat_mod)
AIC(mod)
mod <- glm(DISCARD~offset(log(OBS_KALL)) + CAMS_GEAR_GROUP*HALFOFYEAR + REGION*HALFOFYEAR*MESHGROUP,
           family=poisson(),data=bdat_mod)
AIC(mod)
mod <- glm(DISCARD~offset(log(OBS_KALL+1)) + STRATA -1,
           family=poisson(),data=bdat_mod)
AIC(mod)
summary(mod)

mod <- glm(DISCARD~offset(log(OBS_KALL+1)) + STRATA -1,
           family=Gamma,data=bdat_mod)

ddat_mod <- ddat_focal_gf %>% 
  filter(SUBTRIP_KALL > 0) %>%
  select(DMIS_TRIP_ID,SUBTRIP_KALL,
         STRATA,CAMS_GEAR_GROUP,REGION,HALFOFYEAR,MESHGROUP) %>%
  mutate(
    ...
    NULL=NULL) %>%
  filter(STRATA %in% bdat_mod$STRATA)

ddat_mod <- gf_ex$res %>% filter(STRATA %in% bdat_mod$STRATA) %>%
    mutate(OBS_KALL = SUBTRIP_KALL)

ddat_pred <- data.frame(ddat_mod,d_model=predict(mod,ddat_mod))
sum(exp(ddat_pred$d_model))

sum(exp(ddat_pred$d_model[
  which(!ddat_pred$STRATA %in% bdat_mod$STRATA)])) + sum(dest_strata$disc_est,na.rm=T)

sum(exp(ddat_pred$d_model[
  which(ddat_pred$STRATA %in% bdat_mod$STRATA)]))
