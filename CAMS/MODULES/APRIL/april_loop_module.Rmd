...
# sbrm_species <- read.csv("~/discaRd/CAMS/MODULES/CALENDAR/sbrm_species_022222.csv")
# sbrm_species$SRCE_SCI_NAME <- as.character(sbrm_species$TAXON)
# sbrm_species <- sbrm_species %>% 
#   mutate(across(where(is.character), toupper))
# 
#  species_maps = tbl(bcon, sql("select *
# 												from MAPS.CFG_ITIS")) %>% 
# 	collect() 
# 
# sbrm_join <-  left_join(sbrm_species, species_maps, by="SRCE_SCI_NAME")





 species_nespp3 = c('418','212','801','802','051', '170',
 ...
 species$ITIS_TSN <- as.numeric(species$SRCE_ITIS_TSN)
 species$ITIS_TSN <- as.character(species$ITIS_TSN)
...
species_itis_srce = species$SRCE_ITIS_TSN[i]
#--------------------------------------------------------------------------#
...
ddat_focal_cy = ddat_focal %>% 
  filter(!is.na(LINK1)) %>% 
	mutate(SPECIES_EVAL_DISCARD = case_when(SPECIES_ITIS == species_itis_srce ~ DISCARD
																					)) %>% 
...
d_prev = run_discard(bdat = bdat_prev_cy
										   ...
										   , species_itis = species_itis_srce
											 , stratvars = stratvars
...
trans_rate_df = dest_strata_f %>% 
 ...
 final_table = trans_rate_df %>% 
   ...
   as_tibble() %>% 
 	 	mutate(SPECIES_ITIS_EVAL = species_itis_srce
...
#	mutate(GF_STOCK_DEF = paste0(COMNAME_EVAL, '-', SPECIES_STOCK)) %>% 
	dplyr::rename('STRATA_FULL' = 'STRATA'
								, 'DISCARD_RATE' = 'FINAL_RATE'
...
db_drop_table(con = bcon, table = 'MAPS.CAMS_DISCARD_EXAMPLE_CY_LONGFINSQUID_19', force = F)

dbWriteTable(bcon, name = 'CAMS_DISCARD_EXAMPLE_CY_LONGFINSQUID_19', value = cy_discard_example, overwrite = T)
