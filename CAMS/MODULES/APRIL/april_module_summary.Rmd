---
title: "April Year Module Summary"
...
Exploration of discard estimates from april year module 
...
```sql
	MAPS.CAMS_DISCARD_SCALLOPSEA_2019
	CAMS_GARFO.CAMS_DISCARD_SCALLOPSEA_2019
...
db_scallop = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_SCALLOPSEA_2019")) %>% 
	collect

db_scallop$SPECIES <- "SEA SCALLOP"


db_final <- db_scallop

strat_scallop <- db_scallop %>% 	
...
```{r scallop federal discard table }
...
db_scallop %>% 	
  ...
  dplyr::summarise(SUBTRIP_KALL = sum(SUBTRIP_KALL)
   ...
   ungroup() %>% dplyr::select(STRATA_FULL,
		...
		DT::datatable(caption = 'Discard rates for federal trips by strata for Sea Scallop')

```


```{r scallop state discard table }
...
db_scallop %>% 	
  ...
  dplyr::summarise(SUBTRIP_KALL = sum(SUBTRIP_KALL)
   ...
   ungroup() %>% dplyr::select(STRATA_FULL,
             ...
             # ,CV) 
             dplyr::arrange(desc(DISCARD)) %>%# top_n(10) %>%
		DT::datatable(caption = 'Discard rates for state trips by strata for Sea Scallop')

```



```{r scallop gear fed discard table }
db_scallop %>% filter(FED_OR_STATE == 'FED') %>% group_by(DISCARD_SOURCE, CAMS_GEAR_GROUP, SPECIES_ESTIMATION_REGION) %>% dplyr::summarise(D = sum(CAMS_DISCARD), K = sum(SUBTRIP_KALL), Drate = mean(CAMS_DISCARD_RATE), Dmort = mean(DISC_MORT_RATIO)) %>% arrange(desc(D))%>% top_n(10) %>% dplyr::mutate(across(where(is.numeric), round, 4))%>% mutate(across(ends_with("D"), round, 0))%>%
		DT::datatable(caption = 'Discard totals for federal trips by stock, gear and discard source for Sea Scallop')
```

```{r scallop gear state discard table }
db_scallop %>% filter(FED_OR_STATE == 'STATE') %>% group_by(DISCARD_SOURCE, CAMS_GEAR_GROUP, SPECIES_ESTIMATION_REGION) %>% dplyr::summarise(D = sum(CAMS_DISCARD), K = sum(SUBTRIP_KALL), Drate = mean(CAMS_DISCARD_RATE), Dmort = mean(DISC_MORT_RATIO)) %>% arrange(desc(D))%>% top_n(10) %>% dplyr::mutate(across(where(is.numeric), round, 4))%>% mutate(across(ends_with("D"), round, 0))%>%
		DT::datatable(caption = 'Discard totals for state trips by stock, gear and discard source for Sea Scallop')
```
