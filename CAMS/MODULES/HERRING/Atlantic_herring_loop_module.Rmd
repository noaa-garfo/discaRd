...
#source('~/discaRd/CAMS//R/cams_discard_functions.R') # ask GB about this change
```
...
, case when PERMIT = '000000' then 'STATE'
 ...
 WHERE YEAR >= 2017 
  ...
  ,cl.lat_dd
  ,cl.lon_dd
  ,(select hs.hma from apsd.hma_stat hs where cl.area = hs.area) stat_area_hma
  from maps.cams_landings cl
    where itis_tsn = '161722')
  
  select 
   cos.*
  ,ch.cams_subtrip landings_subt
  ,ch.herr_targ
  , case when ch.herr_targ = 'HERR_TRIP' or cos.species_itis = '161722' then 'HERR'
    else 'NON_HERR' end HERR_FLAG
  , ch.lat_dd
  , ch.lon_dd
...
mnk = run_discard( bdat = bdat_2yrs
			...
			, c_o_tab = ddat_2yr
			, species_itis = species_itis
			, stratvars = stratvars[1]  #"SPECIES_STOCK"   "CAMS_GEAR_GROUP" #AWA just gear_group
			)
...
### AWA test code
summary <- cy_discard_example %>% 
  group_by(HERR_AREA, HERR_FLAG) %>%
  summarize(TOTAL_DISC = sum(DISCARD), AVG_RT = mean(CAMS_DISCARD_RATE))

PS_1A <- cy_discard_example %>%
  filter(STRATA_FULL == 'HERR_1A_120' & DISCARD_SOURCE == 'O')
