...
ustocks = bdat_scal$SPECIES_STOCK 
 %>% unique()
 
BROAD_STOCK_RATE_TABLE = data.frame(BROAD_STOCK_RATE = NULL, CV = NULL, SPECIES_STOCK = NULL)
 
 for(k in ustocks){
 	
get_broad_stock_rate = function(bdat, ddat_focal_sp, ddat_focal, species_itis, stratvars, aidx = 1, stock = 'GOM'){ 	
...
d_broad_stock = run_discard(bdat = btmp
											 ...
											 , species_itis = species_itis
											 , stratvars = stratvars
											 , aidx = aidx  # this makes sure this isn't used..
											 )
...
kk = 1
for(k in bdat_scal$SPECIES_STOCK %>% unique()){
...
```{r substitute scallop trips into non-gf trips, eval = T}
