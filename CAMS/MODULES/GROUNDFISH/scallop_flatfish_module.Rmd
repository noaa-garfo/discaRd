...
```{r substitute trips into non-gf trips}
for(i in 1:length(scal_gf_species$SPECIES_ITIS)){
...
t1  = get(paste0('outlist_df_',sp_itis,'_',FY))
t2 = get(paste0('outlist_df_scal',FY))	
...
t1 %>% group_by(SPECIES_STOCK, ACCESSAREA, TRIP_TYPE) %>% dplyr::summarise(round(sum(DISCARD, na.rm = T)))	

# test the scallop fy 19
t2 %>% 
