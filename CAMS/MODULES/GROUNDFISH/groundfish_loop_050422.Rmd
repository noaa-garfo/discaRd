...
# source(here::here('R/cams_discard_functions.R'))

# setwd(here::here('CAMS/MODULES/GROUNDFISH/'))
...
em_tab = ROracle::dbGetQuery(conn = con_maps, statement = "
			 ...
			 , NMFS_DISCARD_SOURCE
			 , VTRSERNO
			 from CAMS_GF_EM_DELTA_VTR_DISCARD_20_22
			 ") %>% 
	      as_tibble()
...
start_time = Sys.time()
	...
	# for(i in 1:length(scal_gf_species$SPECIES_ITIS)){
		...
		gf_file_dir = here::here('CAMS/MODULES/GROUNDFISH/OUTPUT/')
		...
		#   write.csv(paste0(here::here('CAMS/MODULES/APRIL/OUTPUT/'), sp_itis,'_for_SCAL_YEAR_2019.csv'), row.names = F)
...
res_cams_gear = allgf %>%
	...
	group_by(SPECIES_ESTIMATION_REGION, SPECIES_ITIS_EVAL) %>%
	dplyr::summarise(D = round(sum(DISCARD, na.rm = T))) %>%
	# filter(SPECIES_ITIS %in% species$SPECIES_ITIS) %>%
	pivot_wider(names_from = 'SPECIES_ESTIMATION_REGION', values_from = 'D') %>%
...
allgf %>% 
	...
	group_by(SPECIES_ITIS_EVAL, SPECIES_ESTIMATION_REGION) %>% 
...
nongf_res_list[[1]] %>% 
	group_by(DISCARD_SOURCE, FED_OR_STATE, CAMS_GEAR_GROUP, SPECIES_ESTIMATION_REGION) %>% 
...
nongf_res_list[[1]] %>% 
	group_by(FED_OR_STATE, SPECIES_ESTIMATION_REGION) %>% 
	dplyr::summarise(D = sum(DISCARD, na.rm = T)/2204.62262) %>% 
	pivot_wider(names_from = 'SPECIES_ESTIMATION_REGION', values_from = 'D')

# look at cod by gear group
nongf_res_list[[1]] %>% 
	group_by(FED_OR_STATE, SPECIES_ESTIMATION_REGION, CAMS_GEAR_GROUP) %>% 
	dplyr::summarise(D = sum(DISCARD, na.rm = T)/2204.62262)
#-------------------------------------------------------------
...
db_example %>% 
	...
	slice(1) %>% 
	ggplot()+
	geom_bar(aes(x = SPECIES_ESTIMATION_REGION, y = DISCARD_RATE, fill = DISCARD_SOURCE), stat = 'identity', position = 'dodge')+
	facet_wrap(~COMNAME_EVAL, scales = 'free')+
	theme_light()

db_example %>% 
	# filter(DISCARD_SOURCE != 'O') %>% 
	group_by(SPECIES_ITIS_EVAL, COMNAME_EVAL, DISCARD_SOURCE,SPECIES_ESTIMATION_REGION) %>% 
