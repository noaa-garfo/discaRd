...
for(i in 18:19){
	...
	dbWriteTable(bcon, name = paste0('CAMS_DISCARD_EXAMPLE_GF',i), value =  get(paste0('outlist_df_',FY)), overwrite = T)
...
condat <- config::get(value = "cams_garfo", file = "~/config.yml")
...
for(i in 18:19){
	...
	dbWriteTable(ccon, name = paste0('CAMS_DISCARD_EXAMPLE_GF',i), value =  get(paste0('outlist_df_',FY)), overwrite = T)
	t2 =  Sys.time()
	print(paste0('Year ', i, ' took ', round(difftime(t2, t1, units = "mins"),2), ' minutes to save'))
