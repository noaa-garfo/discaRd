...
trans_rate_df = trans_rate_df %>% 
 ...
 full_strata_table = trans_rate_df %>% 
...
d_prev = run_discard(bdat = bdat_prev_gf
										   ...
										   , species_itis = species_itis
											 ...
											 , aidx = c(1:length(stratvars_assumed))
											 )
...
trans_rate_df = trans_rate_df %>% 
 ...
 #  	select(1:9)
  ...
  names(trans_rate_df) = paste0(names(trans_rate_df), '_a')
...
joined_table = assign_strata(full_strata_table, stratvars_assumed) %>% 
	...
	left_join(., y = trans_rate_df, by = c('STRATA_ASSUMED' = 'STRATA_a')) %>% 
	mutate(COAL_RATE = case_when(n_obs_trip_f >= 5 ~ trans_rate
															 , n_obs_trip_f < 5 & n_obs_trip_p >=5 ~ trans_rate
															 , sum(n_obs_trip_f, n_obs_trip_p) < 5 ~ trans_rate_a
		                           , n_obs_trip_f == 0 & n_obs_trip_p == 0 ~ trans_rate_a))
