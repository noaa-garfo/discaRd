...
setwd('../')
source('R/cams_discard_functions.R')
...
trans_rate_df_pass2 = trans_rate_df_pass2 %>% 
 ...
 #  assumed_strata_table = trans_rate_df %>% 
 #   right_join(., y = d_focal$res, by = 'STRATA') %>% 
 #   as_tibble() %>% 
 # 	 	mutate(SPECIES_ITIS_EVAL = species_itis
 # 				 , COMNAME_EVAL = species$COMNAME[i]
 # 				 , FISHING_YEAR = FY
 # 				 , FY_TYPE = FY_TYPE) %>% 
 # 	   dplyr::rename(ASSUMED_STRATA = STRATA) %>% 
 #  	select(1:9)
...
joined_table = assign_strata(full_strata_table, stratvars_assumed) %>% 
	...
	left_join(x =., y = ARATE_table, by = 'SPECIES_STOCK') %>% 
	mutate(COAL_RATE = case_when(n_obs_trips_f >= 5 ~ trans_rate  # this is an in season rate
															 , n_obs_trips_f < 5 & n_obs_trips_p >=5 ~ final_rate  # this is a final rate taking transition into account
															 , n_obs_trips_f < 5 & n_obs_trips_p < 5 ~ final_rate_a  # this is an final assumed rate taking trasnition into account
		                           )
	) %>% 
...
ddat_focal_gf %>% 
	...
	filter(STRATA == 'WGB and South_100_LM_22_CFF GILLNET RENEWAL_NA_NA_NA_0_0_1') %>% 
