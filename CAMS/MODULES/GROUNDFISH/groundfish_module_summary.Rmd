...
- Discard totals for CAMS are generally lower than DMIS. 
...
- One more significant differnece between DMIS and CAMS groundfish stratification is that Quota Monitoring (DMIS) uses stock area defined biologically, and parses out the designations (STOCK_ID) in tables above AFTER estimating discards. For example, Eastern Georges Bank (EGB) is CAMS Strata. Although DMIS has a separate total for EGB, this is parsed out from the entire Geroges Bank stock. This differential stratification is the most likely explanation for differnces between estimates. Stocks that havea singular STOCK_ID (e.g. Pollock), are fairly close in their DMIS/CAMS discard estimates. 
