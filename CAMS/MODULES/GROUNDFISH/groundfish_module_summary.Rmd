...
db_example = tbl(ccon, sql("
  with gf as ( 
		select a.*
		, b.area
		from cams_garfo.CAMS_DISCARD_EXAMPLE_GF19 a
		left join (
		    select distinct(vtrserno) as vtrserno
		    , area 
		    from maps.cams_catch
		) b
		on (a.vtrserno = b.vtrserno)
		)
...
for(j in species$SPECIES_ITIS){
	...
	comp_list[[ii]] = comp_list[[ii]] %>% 
		...
		group_by(COMNAME_EVAL, CAMS_STOCK_AREA, DMIS_STOCK_ID) %>% 
		dplyr::summarise(CAMS_DISCARD = round(sum(DISCARD, na.rm = T))) %>% 
		filter(!is.na(COMNAME_EVAL))
...
dmis_comp = allgf_comp %>% 
	group_by(COMNAME_EVAL, DMIS_STOCK_ID) %>% 
	...
	dplyr::rename('COMMON_NAME' = 'COMNAME_EVAL')
...
```{r comparison of gear stratifications}
...
- Discard totals for CAMS are generally much lower than DMIS. 
	...
	- A potentially major diversion between DMIS and CAMS groundfish stratification is that Quota Monitoring (DMIS) uses stock area defined biologically and parses out the designations (STOCK_ID) in tables above AFTER estimating discards. It is not used as a stratifcation variable for the species. For example, Eastern Georges Bank (EGB) is CAMS Strata. Although DMIS has a separate total for EGB, this is parsed out from the entire Geroges Bank stock. This differential stratification is the most likely source that explains differnces in estimates. Stocks that havea singular STOCK_ID (e.g. Pollock), are fairly close in their DMIS/CAMS discard estimates. 
