...
# 	slice(1) %>% 
# 	ggplot()+
# 	geom_bar(aes(x = SPECIES_ESTIMATION_REGION, y = CAMS_DISCARD_RATE, fill = DISCARD_SOURCE), stat = 'identity', position = 'dodge')+
...
db_example %>% 
	# filter(DISCARD_SOURCE != 'O') %>% 
	group_by(ITIS_TSN, COMMON_NAME, DISCARD_SOURCE,SPECIES_ESTIMATION_REGION) %>% 
...
")
 ...
 t2 %>% 
 	...
 	mutate(CAMS_STOCK_AREA = paste(COMMON_NAME, SPECIES_ESTIMATION_REGION, sep = '_')) %>% 
...
t3 = tbl(con_maps, sql("select *
	...
	filter(!is.na(SPECIES_ESTIMATION_REGION)) %>% 
 ...
 t1 %>% 
 	...
 	group_by(STOCK_ID.y, SPECIES_ESTIMATION_REGION) %>%
 	# head() %>% 
 	dplyr::summarise(KALL_DMIS = round(sum(POUNDS, na.rm = T))) %>%
 	left_join(., t3, by = 'SPECIES_ESTIMATION_REGION') %>% 
 	collect() %>% 
 	mutate(SPECIES_ITIS = species_itis) %>% 
 	dplyr::rename('STOCK_ID_DMIS' = 'STOCK_ID.y', 'STOCK_ID_CAMS' = 'SPECIES_ESTIMATION_REGION')
...
non_gf = tbl(con_maps, sql("
    ...
    '630979'
     ...
     group by ITIS_TSN, COMMON_NAME, DISCARD_SOURCE,SPECIES_ESTIMATION_REGION
...
")) %>% 
	...
	ggplot(non_gf)+
	geom_bar(aes(x = SPECIES_ESTIMATION_REGION, y = CAMS_DISCARD, fill = DISCARD_SOURCE), stat = 'identity', position = 'dodge')+
	facet_wrap(~COMMON_NAME, scales = 'free')+
	theme_light()
...
```{r non-gf trips table}
    ...
    select COMMON_NAME, SPECIES_ESTIMATION_REGION, FED_OR_STATE
    , sum(CAMS_DISCARD) as CAMS_DISCARD
    ...
    '630979'
     ...
     group by COMMON_NAME,SPECIES_ESTIMATION_REGION, FED_OR_STATE
...
```{r scallop trips table, eval = T}
    ...
    select COMMON_NAME, SPECIES_ESTIMATION_REGION, SCALLOP_AREA, FY
    , sum(CAMS_DISCARD) as CAMS_DISCARD
    , sum(subtrip_kall) as KALL
    ...
    '630979'
     ...
     group by COMMON_NAME,SPECIES_ESTIMATION_REGION, SCALLOP_AREA, FY
