...
Exploration of discard estimates from calendar year module developed for 
QM and SBRM species
...
```sql
	MAPS.CAMS_DISCARD_EXAMPLE_CY_BLACKSEABASS_19
	CAMS_GARFO.CAMS_DISCARD_EXAMPLE_CY_BLACKSEABASS_19
	
	MAPS.CAMS_DISCARD_EXAMPLE_CY_BLUEFISH_19
	CAMS_GARFO.CAMS_DISCARD_EXAMPLE_CY_BLUEFISH_19
	
	MAPS.CAMS_DISCARD_EXAMPLE_CY_BUTTERFISH_19
	CAMS_GARFO.CAMS_DISCARD_EXAMPLE_CY_BUTTERFISH_19
	
	MAPS.CAMS_DISCARD_EXAMPLE_CY_LONGFINSQUID_19
	CAMS_GARFO.CAMS_DISCARD_EXAMPLE_CY_LONGFINSQUID_19
	
	MAPS.CAMS_DISCARD_EXAMPLE_CY_SHORTFINSQUID_19
	CAMS_GARFO.CAMS_DISCARD_EXAMPLE_CY_SHORTFINSQUID_19
	
	MAPS.CAMS_DISCARD_EXAMPLE_CY_MACKEREL_19
	CAMS_GARFO.CAMS_DISCARD_EXAMPLE_CY_MACKEREL_19
...
Highest discrepancies for total discards with CAMS vs. QM ACL 2019 are with Butterfish, Mackerel and Black Sea Bass.  
...
discard %>% filter(Source %in% c("State")) %>% filter(Species %ni% c("Summer flounder")) %>% dplyr::select(Species, Discards) %>%
		DT::datatable(caption = 'Total discards by species for state trips')

```

Since discards from state trips were not estimated in 2019 QM ACL we will need to compare these with estimates from the stock assessments. Are lower than expected (except for bluefish).


```{r make diagnostic plot kall, eval = T, echo = F, fig.cap = "2019 KALL Totals by Method and Source", fig.height = 8, fig.width = 12}

Kall <- read.csv("~/discaRd/CAMS/MODULES/CALENDAR/January_kall.csv")
...
db_blackseabass = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_EXAMPLE_CY_BLACKSEABASS_19")) %>% 
	collect

db_blackseabass$SPECIES <- "BLACK SEA BASS"

db_bluefish = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_EXAMPLE_CY_BLUEFISH_19")) %>% 
	collect

db_bluefish$SPECIES <- "BLUEFISH"

db_butterfish = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_EXAMPLE_CY_BUTTERFISH_19")) %>% 
	collect

db_butterfish$SPECIES <- "BUTTERFISH"

db_longfinsquid = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_EXAMPLE_CY_LONGFINSQUID_19")) %>% 
	collect

db_longfinsquid$SPECIES <- "LONGFIN SQUID"

db_shortfinsquid = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_EXAMPLE_CY_SHORTFINSQUID_19")) %>% 
	collect

db_shortfinsquid$SPECIES <- "SHORTFIN SQUID"

db_mackerel = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_EXAMPLE_CY_MACKEREL_19")) %>% 
	collect

db_mackerel$SPECIES <- "MACKEREL"

db_final <- rbind(db_blackseabass,db_bluefish,
                  db_butterfish, db_longfinsquid,
                  db_shortfinsquid, db_mackerel)
...
strat_blackseabass <- db_blackseabass %>% 	
  ...
  dplyr::summarise(SUBTRIP_KALL = sum(SUBTRIP_KALL)
                       , DISCARD_RATE = round(mean(DISCARD_RATE), digits=5)
...
strat_final <- rbind(strat_blackseabass, strat_bluefish,
                     strat_butterfish, strat_longfinsquid,
                     strat_shortfinsquid, strat_mackerel)
  
  
strat_final %>% 
	filter(DISCARD_SOURCE != 'O' & DISCARD_RATE >0.0000001) %>% 
	dplyr::group_by(SPECIES, STRATA, DISCARD_SOURCE, TRIP_TYPE) %>% 
	#slice(1) %>% 
	ggplot()+
	geom_histogram(aes(x = DISCARD_RATE, fill =  DISCARD_SOURCE, group= TRIP_TYPE), bins=60, position = "dodge") +
	facet_wrap(~SPECIES, scale="free")+
	theme_light()
```

Only includes strata with discard rates >0.0000001. 


```{r plot 1, echo = F, fig.cap = "Total Discard Estimates by Species, Stock and Discard Source" , fig.height = 8, fig.width = 12}
db_final %>% 
	# filter(DISCARD_SOURCE != 'O') %>% 
	group_by(SPECIES, SPECIES_ITIS_EVAL, DISCARD_SOURCE,SPECIES_STOCK) %>% 
...
db_blackseabass %>% 	
  ...
  mutate(STRATA = case_when(DISCARD_SOURCE == "I" ~ STRATA_FULL, DISCARD_SOURCE == "A" ~ STRATA_ASSUMED)) %>% ungroup() %>% dplyr::select(STRATA 
             ...
             # ,CV) 
             dplyr::arrange(desc(DISCARD)) %>% 
		DT::datatable(caption = 'Discard rates by Strata for Black Sea Bass')
```



```{r bluefish discard table }
 ...
 db_bluefish %>% 	
  ...
  mutate(STRATA = case_when(DISCARD_SOURCE == "I" ~ STRATA_FULL, DISCARD_SOURCE == "A" ~ STRATA_ASSUMED)) %>% ungroup() %>% dplyr::select(STRATA 
		...
		DT::datatable(caption = 'Discard rates by Strata for Bluefish based on federal trips')
```

```{r butterfish discard table }
...
db_butterfish %>% 	
  ...
  mutate(STRATA = case_when(DISCARD_SOURCE == "I" ~ STRATA_FULL, DISCARD_SOURCE == "A" ~ STRATA_ASSUMED)) %>% ungroup() %>% dplyr::select(STRATA 
             ...
             # ,CV) 
             dplyr::arrange(desc(DISCARD)) %>% top_n(10) %>%
		DT::datatable(caption = 'Discard rates by Strata for Butterfish based on federal trips')
```

```{r longfinsquid discard table }
...
db_longfinsquid %>% 	
  ...
  mutate(STRATA = case_when(DISCARD_SOURCE == "I" ~ STRATA_FULL, DISCARD_SOURCE == "A" ~ STRATA_ASSUMED)) %>% ungroup() %>% dplyr::select(STRATA 
             ...
             # ,CV) 
             dplyr::arrange(desc(DISCARD)) %>% top_n(10) %>% 
		DT::datatable(caption = 'Discard rates by Strata for Longfin Squid based on federal trips')
```

```{r shortfinsquid discard table }
...
db_shortfinsquid %>% 	
  ...
  mutate(STRATA = case_when(DISCARD_SOURCE == "I" ~ STRATA_FULL, DISCARD_SOURCE == "A" ~ STRATA_ASSUMED)) %>% ungroup() %>% dplyr::select(STRATA 
             ...
             # ,CV) 
             dplyr::arrange(desc(DISCARD)) %>% top_n(10) %>% 
		DT::datatable(caption = 'Discard rates by Strata for Shortfin Squid based on federal trips')
```

```{r mackerel discard table }
...
db_mackerel %>% 	
  ...
  mutate(STRATA = case_when(DISCARD_SOURCE == "I" ~ STRATA_FULL, DISCARD_SOURCE == "A" ~ STRATA_ASSUMED)) %>% ungroup() %>% dplyr::select(STRATA 
             ...
             # ,CV) 
             dplyr::arrange(desc(DISCARD)) %>% top_n(10) %>%
		DT::datatable(caption = 'Discard rates by Strata for Mackerel based on federal trips')
```
