...
```sql
	...
	MAPS.CAMS_DISCARD_EXAMPLE_CY_FLUKE_19
	CAMS_GARFO.CAMS_DISCARD_EXAMPLE_CY_FLUKE_19
...
discard <- discard %>% filter(Source %in% c("Federal")) 
...
db_fluke = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_EXAMPLE_CY_FLUKE_19")) %>% 
	collect
...
db_fluke %>% 	
  ...
  mutate(STRATA = case_when(DISCARD_SOURCE == "I" ~ STRATA_FULL, DISCARD_SOURCE == "A" ~ STRATA_ASSUMED)) %>% ungroup() %>% dplyr::select(STRATA 
             ...
             # ,CV) 
             dplyr::arrange(desc(DISCARD)) %>% top_n(10) %>%
		DT::datatable(caption = 'Discard rates by Strata for Fluke based on federal trips')
```
### Appendix
