...
```sql
	MAPS.CAMS_DISCARD_EXAMPLE_CY_DOGFISH_19
	CAMS_GARFO.CAMS_DISCARD_EXAMPLE_CY_DOGFISH_19
	
	MAPS.CAMS_DISCARD_EXAMPLE_CY_MONKFISH_19
	CAMS_GARFO.CAMS_DISCARD_EXAMPLE_CY_MONKFISH_19
	
	MAPS.CAMS_DISCARD_EXAMPLE_CY_OFFSHOREHAKE_19
	CAMS_GARFO.CAMS_DISCARD_EXAMPLE_CY_OFFSHOREHAKE_19
	
	MAPS.CAMS_DISCARD_EXAMPLE_CY_REDHAKE_19
	CAMS_GARFO.CAMS_DISCARD_EXAMPLE_CY_REDHAKE_19
	
	MAPS.CAMS_DISCARD_EXAMPLE_CY_SILVERHAKE_19
	CAMS_GARFO.CAMS_DISCARD_EXAMPLE_CY_SILVERHAKE_19
...
Here, I combined the separate QM estimates for northern and southern red hake for and the northern silver and southern whiting for silver hake (need a check on that).  
...
I'm assuming these QM estimates don't include all relevant strata for kall estimates?
...
db_dogfish = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_EXAMPLE_CY_DOGFISH_19")) %>% 
	collect

db_dogfish$SPECIES <- "DOGFISH"

db_offshore = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_EXAMPLE_CY_OFFSHOREHAKE_19")) %>% 
	collect

db_offshore$SPECIES <- "OFFSHORE HAKE"

db_red = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_EXAMPLE_CY_REDHAKE_19")) %>% 
	collect

db_red$SPECIES <- "RED HAKE"

db_silver = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_EXAMPLE_CY_SILVERHAKE_19")) %>% 
	collect

db_silver$SPECIES <- "SILVER HAKE"

db_monkfish = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_EXAMPLE_CY_MONKFISH_19")) %>%
	collect
...
strat_dogfish <- db_dogfish %>% 	
  ...
  group_by(SPECIES, STRATA_FULL, STRATA_ASSUMED, DISCARD_SOURCE, FED_OR_STATE) %>%
...
```{r plot 1, echo = F, fig.cap = "Total Discard Estimates by Species, Stock and Discard Source for State trips" , fig.height = 8, fig.width = 12}
db_final %>% 
	# filter(DISCARD_SOURCE != 'O')&
...
```{r dogfish discard table }

db_dogfish %>% 	
  filter(DISCARD_SOURCE != 'O' & FED_OR_STATE=="FED") %>%
  group_by(STRATA_FULL, STRATA_ASSUMED, DISCARD_SOURCE, FED_OR_STATE) %>%
  dplyr::summarise(SUBTRIP_KALL = sum(SUBTRIP_KALL)
                       , CAMS_DISCARD_RATE = round(mean(CAMS_DISCARD_RATE), digits=5)
                       , DISC_MORT_RATIO = mean(DISC_MORT_RATIO)
                      # , CV = mean(CV, na.rm=TRUE)
  ...
  mutate(STRATA = case_when(DISCARD_SOURCE == "I" ~ STRATA_FULL, DISCARD_SOURCE == "A" ~ STRATA_ASSUMED)) %>% ungroup() %>% dplyr::select(STRATA 
		...
		DT::datatable(caption = 'Discard rates by Strata for Dogfish')

```

```{r monkfish discard table }
...
db_monkfish %>% 	
  ...
  dplyr::summarise(SUBTRIP_KALL = sum(SUBTRIP_KALL)
                      ...
                      # , CV = mean(CV, na.rm=TRUE)
                       , DISCARD = round(sum(DISCARD))) %>%
                      # , N_OBS_TRIPS_F = mean(N_OBS_TRIPS_F)) %>% 
  mutate(STRATA = case_when(DISCARD_SOURCE == "I" ~ STRATA_FULL, DISCARD_SOURCE == "A" ~ STRATA_ASSUMED,
DISCARD_SOURCE == "B" ~ STRATA_ASSUMED))%>% ungroup() %>% dplyr::select(STRATA 
             # ,N_OBS_TRIPS_F
              ,CAMS_DISCARD_RATE
              ,SUBTRIP_KALL
              ,DISC_MORT_RATIO
              ,DISCARD) %>%
             # ,CV) 
             dplyr::arrange(desc(DISCARD)) %>%top_n(12) %>%
		DT::datatable(caption = 'Discard rates by Strata for Monkfish')
```

```{r offshore hake discard table }
...
db_offshore %>% 	
  ...
  mutate(STRATA = case_when(DISCARD_SOURCE == "I" ~ STRATA_FULL, DISCARD_SOURCE == "A" ~ STRATA_ASSUMED)) %>% ungroup() %>% dplyr::select(STRATA 
             ...
             # ,CV) 
             dplyr::arrange(desc(DISCARD)) %>% top_n(10) %>%
		DT::datatable(caption = 'Discard rates by Strata for Offshore Hake')
```

```{r redhake discard table }
...
db_red %>% 	
  ...
  mutate(STRATA = case_when(DISCARD_SOURCE == "I" ~ STRATA_FULL, DISCARD_SOURCE == "A" ~ STRATA_ASSUMED)) %>% ungroup() %>% dplyr::select(STRATA 
             ...
             # ,CV) 
             dplyr::arrange(desc(DISCARD)) %>% top_n(10) %>%
		DT::datatable(caption = 'Discard rates by Strata for Red Hake')
```

```{r silver hake discard table }
...
db_silver %>% 	
  ...
  mutate(STRATA = case_when(DISCARD_SOURCE == "I" ~ STRATA_FULL, DISCARD_SOURCE == "A" ~ STRATA_ASSUMED)) %>% ungroup() %>% dplyr::select(STRATA 
             ...
             # ,CV) 
             dplyr::arrange(desc(DISCARD)) %>% top_n(10) %>%
		DT::datatable(caption = 'Discard rates by Strata for Silver Hake')
```
