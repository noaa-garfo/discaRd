...
for(k in ustocks){
	BROAD_STOCK_RATE_TABLE[[kk]] = get_broad_stock_rate(bdat = bdat_cy
											 , ddat_focal_sp = ddat_focal_cy
...
print(paste('RUNTIME: ', round(difftime(t2, t1, units = "mins"),2), ' MINUTES',  sep = ''))
 ...
 joined_table$FINAL_RATE[is.na(joined_table$FINAL_RATE)]<-0
 
 joined_table$COMMON_NAME <- species$COMMON_NAME[i]
 
 sum(joined_table$DISCARD, na.rm=TRUE)
...
# dbWriteTable(bcon, name = 'CAMS_DISCARD_REDHAKE_SOUTHERN_50_LM_ALL_ALL', value = caless, overwrite = T)
...
caless <- cy_discard_example %>% filter(STRATA_FULL =='SOUTHERN_50_LM_all_all' & DISCARD_SOURCE=='O')
