...
CAMS_GEAR_GROUP = bdat_cy$CAMS_GEAR_GROUP %>% unique()

for(k in 1:length(ustocks)){
  for(i in 1:length(CAMS_GEAR_GROUP)){
	BROAD_STOCK_RATE_TABLE[[kk]] = get_broad_stock_gear_rate(bdat = rbind(bdat_prev_cy,bdat_cy)
											 , ddat_focal_sp = rbind(ddat_prev_cy,ddat_focal_cy)
											 , ddat_focal = rbind(ddat_prev,ddat_focal)
											 , species_itis = species_itis
