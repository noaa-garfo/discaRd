...
setwd("~/PROJECTS/discaRd/CAMS/MODULES/APRIL/")
...
```{r run sbrm calendar year species RMD as a script, eval = T}
...
common_name <- c('CUSK'
 ...
 species <- species %>% slice(rep(row_number(), length(itis_sbrm)))
 
 species$COMMON_NAME <- common_name
 
 species$SPECIES_ITIS <- itis_sbrm
 
 species$ITIS_TSN <- itis_sbrm
 
 
# make sure the folder is correct 
setwd('~/PROJECTS/discaRd/CAMS/MODULES/SBRM/') 

# make a script from RMD.. 
knitr::purl('sbrm_loop_062122.Rmd', documentation = 0)

# Define year to run
for(jj in 2018:2021){
	FY <- jj
	FY_TYPE = 'JANUARY START'
  source('sbrm_loop_062122.R')  # this is the script created via purl just above
}
...
# parse_upload_discard(con_maps, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/CALENDAR/OUTPUT/', FY = FY)
#
# }
# parse_upload_discard(con_maps, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/MAY/OUTPUT/', FY = FY)
#
# parse_upload_discard(con_maps, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/NOVEMBER/OUTPUT/', FY = FY)
#
# parse_upload_discard(con_maps, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/MARCH/OUTPUT/', FY = FY)
#
# parse_upload_discard(con_maps, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/APRIL/OUTPUT/', FY = FY) # be careful of this one.. groundfish should not be uploaded from here!

parse_upload_discard(con_maps, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/APRIL/SCALLOP', FY = FY) # Scallop only

parse_upload_discard(con_maps, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/SBRM/OUTPUT', FY = FY) 

# parse_upload_discard(con_maps, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/GROUNDFISH/OUTPUT/', FY = FY)	# loading this last ensures that yellowtail and windowpane are correct
}
...
# parse_upload_discard(con_cams, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/CALENDAR/OUTPUT/', FY = FY)
#
# }
# parse_upload_discard(con_cams, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/MAY/OUTPUT/', FY = FY)
#
# parse_upload_discard(con_cams, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/NOVEMBER/OUTPUT/', FY = FY)
#
# parse_upload_discard(con_cams, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/MARCH/OUTPUT/', FY = FY)
#
# parse_upload_discard(con_cams, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/APRIL/OUTPUT/', FY = FY) # be careful of this one.. groundfish should not be uploaded from here!

parse_upload_discard(con_cams, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/APRIL/SCALLOP/', FY = FY) # Scallop only

parse_upload_discard(con_maps, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/SBRM/OUTPUT', FY = FY) 

# parse_upload_discard(con_cams, filepath = '~/PROJECTS/discaRd/CAMS/MODULES/GROUNDFISH/OUTPUT/', FY = FY)	# loading this last ensures that yellowtail and windowpane are correct
#
}
