---
title: "November loop for discards"
output:
  pdf_document: 
    highlight: zenburn
    toc: yes
...
```{r global_options, include=FALSE, purl = F}
...
```{r subset tables join and run old, eval = F, purl = F}
joined_table %>% 
...
dbWriteTable(con_maps, name = 'CAMS_DISCARD_GOLDENTILEFISH_20', value = cy_discard_example, overwrite = T)

cy_discard_example %>% filter(FED_OR_STATE == 'FED') %>% group_by(DISCARD_SOURCE, CAMS_GEAR_GROUP, SPECIES_ESTIMATION_REGION) %>% dplyr::summarise(D = sum(DISCARD), K = sum(SUBTRIP_KALL), Drate = mean(CAMS_DISCARD_RATE), Dmort = mean(DISC_MORT_RATIO)) %>% arrange(desc(D))%>% top_n(10) %>% dplyr::mutate(across(where(is.numeric), round, 4))%>%
