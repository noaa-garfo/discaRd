---
title: "SBRM January loop for discards"
output:
  pdf_document: 
    highlight: zenburn
    toc: yes
...
STOCK_AREAS = tbl(con_maps, sql('select * from MAPS.CAMS_STATAREA_STOCK')) %>%
  # filter(NESPP3 == species_nespp3) %>%  # removed  & SPECIES_ESTIMATION_REGION == species_estimation_region
	dplyr::filter(SPECIES_ITIS == alewife) %>%
    collect() %>% 
  ...
  mutate(LIVE_POUNDS = SUBTRIP_KALL
   ...
   left_join(., y = CAMS_DISCARD_MORTALITY_STOCK
	...
	dplyr::select(-SPECIES_ITIS.y, -GEARCODE.y) %>% 
		dplyr::rename(COMMON_NAME= species$COMMON_NAME,SPECIES_ITIS = 'SPECIES_ITIS.x', NESPP3 = 'NESPP3.x',
	              GEARCODE = 'GEARCODE.x') %>% 
...
ddat_prev <- all_dat %>% 
  ...
  mutate(LIVE_POUNDS = SUBTRIP_KALL
   ...
   left_join(., y = CAMS_DISCARD_MORTALITY_STOCK
	...
	dplyr::select(-SPECIES_ITIS.y, -GEARCODE.y) %>% 
	dplyr::rename(COMMON_NAME= COMMON_NAME= species$COMMON_NAME,SPECIES_ITIS = 'SPECIES_ITIS.x', NESPP3 = 'NESPP3.x',
	              GEARCODE = 'GEARCODE.x') %>% 
  relocate('COMMON_NAME','SPECIES_ITIS','NESPP3','SPECIES_ESTIMATION_REGION','CAMS_GEAR_GROUP','DISC_MORT_RATIO')
...
# 	       
 ...
 joined_table$COMMON_NAME <- species$COMMON_NAME[i]
 
 fst::write_fst(x = joined_table, path = paste0('~/PROJECTS/discaRd/CAMS/MODULES/SBRM/OUTPUT/discard_est_', species_itis, '_trips', FY,'.fst'))
...
cy_discard_example %>% filter(FED_OR_STATE == 'STATE') %>% group_by(DISCARD_SOURCE, CAMS_GEAR_GROUP, SPECIES_ESTIMATION_REGION) %>% dplyr::summarise(D = sum(DISCARD), K = sum(SUBTRIP_KALL), Drate = mean(CAMS_DISCARD_RATE), Dmort = mean(DISC_MORT_RATIO)) %>% arrange(desc(D))%>% top_n(10) %>% dplyr::mutate(across(where(is.numeric), round, 4))%>%
