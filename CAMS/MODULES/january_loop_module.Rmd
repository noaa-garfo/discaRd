...
trans_rate_df = dest_strata_f %>% 
 ...
 final_table$disc_est_byrate <-final_table$SUBTRIP_KALL*final_table$FINAL_RATE*final_table$DISC_MORT_RATIO
 
 sum(final_table$KALL.y, na.rm=TRUE)
 
 sum(final_table$disc_est_byrate, na.rm=TRUE)
 
 observed <- final_table %>% filter(DISCARD_SOURCE =="O")
 x <- sum(observed$OBS_DISCARD)
 
 unobserved <- final_table %>% filter(DISCARD_SOURCE !="O")
 y <- sum(unobserved$disc_est_byrate, na.rm = TRUE)
