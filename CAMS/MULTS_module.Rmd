...
CAMS_GEAR_STRATA = tbl(bcon, sql('  select *   from APSD.CAMS_GEARCODE_STRATA')) %>% collect()

```



```{r subset trip data from oracle table for focal year, eval = F}
#---------------------------------------------------------------------------------------#
...
# most likely not using this approach... 
...
```{r pick species to run}
#  all done in SQL above!! 

# species_nespp3 = '081'

# implemented in SQL in first chunk... 


# assign trips their appropriate gear groupings.. 
# done in SQL above.. 

# ddat_focal <- CAMS_GEAR_STRATA %>% 
#   filter(NESPP3 == species_nespp3) %>% 
#   mutate(GEARCODE = VTR_GEAR_CODE) %>% 
#   mutate(CAMS_GEAR_GROUP = case_when(GEARCODE == 'HND' ~ '0', GEARCODE != 'HND' ~ CAMS_GEAR_GROUP)) %>% 
#   group_by(GEARCODE) %>% 
#   dplyr::summarise(CAMS_GEAR_GROUP = max(CAMS_GEAR_GROUP)) %>% 
#   right_join(ddat_focal, by = 'GEARCODE')

# ddat_prev <- CAMS_GEAR_STRATA %>% 
#   filter(NESPP3 == species_nespp3) %>% 
#   mutate(GEARCODE = VTR_GEAR_CODE) %>% 
#   mutate(CAMS_GEAR_GROUP = case_when(GEARCODE == 'HND' ~ '0', GEARCODE != 'HND' ~ CAMS_GEAR_GROUP)) %>% 
#   group_by(GEARCODE) %>% 
#   dplyr::summarise(CAMS_GEAR_GROUP = max(CAMS_GEAR_GROUP)) %>% 
#   right_join(ddat_prev, by = 'GEARCODE')
...
ddat_focal <- ddat_focal %>% 
	  mutate(STRATA2 = paste(CAMS_GEAR_GROUP, MESHGROUP, CAREA, SECTOR_ID, sep = '_')
	  			 , SEADAYS = 0
	  			 , NESPP3 = NESPP3_FINAL
	  			 )
