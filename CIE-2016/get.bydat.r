...
get.bydat <-
	function(fdat, load = T, bspec = 366, aggfact = 'LINK1', catch_disp = c(0,1)[1]) {
		...
		bdat = dat |>
		  group_by(aggfact) |>
		  dplyr::summarise(BYCATCH = sum(CALCLIVEWT[CATDISP %in% catch_disp &
		                                                NESPP3 %in% bspec], na.rm = T)
		                   ,  KALL = sum(CALCLIVEWT[CATDISP == 1], na.rm =
		                                   T)
		                   , FY = FY[1]
		                   , yday = yday[1]
		                   , fday = fday[1]
				               , SEADAYS = as.numeric(DATELAND[1] - DATESAIL[1])
				               )
