...
get.weighted.cv <- function(bydat, dest2){
	...
	targall = bydat |>
	  group_by(STRATA) |>
	  group_modify(~ data.frame(cochran_calc_ss(l_N = .x$N[1], l_CVtarg = seq(0,1,.01))))
	...
	w = targall |>
	  group_by(STRATA) |>
	  dplyr::summarise(weight = sqrt(sum((K^2)*RE_var, na.rm=TRUE)/sum(D, na.rm=TRUE)))

	# wtsamp = ddply(targall, 'CV_TARG', function(x) data.frame(REQ_COV_WT = weighted.mean(x$REQ_COV, w = x$D, na.rm = T)))

	wtsamp = targall |>
	  group_by(CV_TARG) |>
	  dplyr::summarise(REQ_COV_WT = weighted.mean(REQ_COV, w = D, na.rm = T))
	...
	samptot = targall |>
	  group_by('CV_TARG') |>
	  dplyr::summarise(SAMPTOT = sum(REQ_SAMPLES_WT, na.rm=T))
