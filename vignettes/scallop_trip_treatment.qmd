...
`CAMS_DISCARD_ALL_YEARS` is categorized by species and fishing year of that species. The stratification elements within the `STRATA_USED` and `STRATA_USED_DESC` columns normally will match the fishing year for that species. In the case of federal scallop trips, they do not.

![Example of a diagnostic run of yellowtail flounder highlighting differences in FY_TYPE and STRATA_USED_DESC for scallop trip strata. Note that FY (Fishing Year) refers to the species evaluated (yellowtail)](images/scallop_trips_ytf_example.PNG)
