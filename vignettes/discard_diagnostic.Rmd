...
library(remotes)
remotes::install_github("NOAA-Fisheries-Greater-Atlantic-Region/discaRd", ref = 'demo')
...
start_year = 2020
end_year = 2022 #year(today())

dat = get_catch_obs_nefsc(con, start_year, end_year)
# dat = get_catch_obs_nefsc(con, start_year, end_year) # if at NEFSC, use this
...
species <- tbl(con, sql("
  select *
  from CAMS_GARFO.CFG_DISCARD_RUNID
  ")) %>%
...
mydiscard$discard_summary %>% 
	# knitr::kable()
	DT::datatable(filter = 'top', caption = 'Summary from run')
...
d_summary %>% 
	DT::datatable(filter = 'top', caption = 'Summary built from trips')
	# knitr::kable()
...
```{r discard alll years summary}


DYS = tbl(con, sql('select * from CAMS_GARFO.WG_CAMS_DISCARD_ALL_YEARS where FY = 2021 and ITIS_TSN = 167687'))

dys_summary = DYS %>% 
 filter(FED_OR_STATE == 'FED') %>% 
	collect() %>%
	group_by(STRATA_USED, DISCARD_SOURCE, SPECIES_STOCK, CAMS_GEAR_GROUP, MESH_CAT, TRIPCATEGORY, ACCESSAREA) %>% 
	...
	arrange(desc(D)) %>% 
	mutate(D_mt = round(D/2204.62262))

dys_summary %>% 
	DT::datatable(filter = 'top', caption = 'Summary from discard_all_years table')
...
discard_wtf$discard_summary %>% 
	DT::datatable(filter = 'top', caption = 'Witch flounder, discard summary from run (all trips)')
...
gftrips_wtf_summary %>% 
	DT::datatable(filter = 'top', caption = 'Witch flounder, groundfish trips summary')

nongf_trips_wtf_summary %>% 
	DT::datatable(filter = 'top', caption = 'Witch flounder, non groundfish trips summary')
