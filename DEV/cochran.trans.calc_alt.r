#' Calculate the in-season cumulative bycatch with a transition rate
#' 
#' Apply transition rate to in-season cumulative Cochran bycatch estimates for a set of trips and observed trips, according to a strata definition
...
#' @details The cumulative rate can be calculated without any transition rate by using method = "none".
#' @return A list of Cochran estimates by strata, Discards, Total Discard in the fishery (adjusted by transition rate)
#' @export
#'
#' @examples
cochran.trans.calc_alt <- function(bydat_focal, trips_focal,
  ...
  cochran_prev  <- get.cochran.ss.by.strat_alt(
  ...
  cochran_focal <- llply(days, 
                         function(x) get.cochran.ss.by.strat_alt(
