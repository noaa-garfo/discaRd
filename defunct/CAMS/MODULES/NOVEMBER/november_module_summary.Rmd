...
```sql
	MAPS.CAMS_DISCARD_GOLDENTILEFISH_19
	CAMS_GARFO.CAMS_DISCARD_GOLDENTILEFISH_19
	
	MAPS.CAMS_DISCARD_BLUELINETILEFISH_19
	CAMS_GARFO.CAMS_DISCARD_BLUELINETILEFISH_19
...
db_goldentilefish = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_TILEFISHGOLDEN_2019")) %>% 
	collect

db_goldentilefish$SPECIES <- "GOLDENTILEFISH"

db_bluelinetilefish = tbl(bcon, sql("select * from MAPS.CAMS_DISCARD_TILEFISHBLUELINE_2019")) %>% 
	collect
...
```{r goldentilefish federal discard table }
...
db_goldentilefish %>% 	
  ...
  dplyr::summarise(SUBTRIP_KALL = sum(SUBTRIP_KALL)
   ...
   ungroup() %>% dplyr::select(STRATA_FULL,
		...
		DT::datatable(caption = 'Discard rates for federal trips by strata for Goldentilefish')

```


```{r goldentilefish state discard table }
...
db_goldentilefish %>% 	
  ...
  dplyr::summarise(SUBTRIP_KALL = sum(SUBTRIP_KALL)
   ...
   ungroup() %>% dplyr::select(STRATA_FULL,
             ...
             # ,CV) 
             dplyr::arrange(desc(DISCARD)) %>%# top_n(10) %>%
		DT::datatable(caption = 'Discard rates for state trips by strata for goldentilefish')

```


```{r bluelinetilefish fed discard table }
...
db_bluelinetilefish %>% 	
  ...
  dplyr::summarise(SUBTRIP_KALL = sum(SUBTRIP_KALL)
   ...
   ungroup() %>% dplyr::select(STRATA_FULL,
             ...
             # ,CV) 
             dplyr::arrange(desc(DISCARD)) %>%# top_n(10) %>%
		DT::datatable(caption = 'Discard rates by Strata for Bluelinetilefish')
```

```{r bluelinetilefish state discard table }
...
```{r goldentilefish gear fed discard table }
db_goldentilefish %>% filter(FED_OR_STATE == 'FED') %>% group_by(DISCARD_SOURCE, CAMS_GEAR_GROUP, SPECIES_ESTIMATION_REGION) %>% dplyr::summarise(D = sum(CAMS_DISCARD), K = sum(SUBTRIP_KALL), Drate = mean(CAMS_DISCARD_RATE), Dmort = mean(DISC_MORT_RATIO)) %>% arrange(desc(D))%>% top_n(10) %>% dplyr::mutate(across(where(is.numeric), round, 4))%>% mutate(across(ends_with("D"), round, 0))%>%
		DT::datatable(caption = 'Discard totals for federal trips by stock, gear and discard source for Goldentilefish')
```

```{r goldentilefish gear state discard table }
db_goldentilefish %>% filter(FED_OR_STATE == 'STATE') %>% group_by(DISCARD_SOURCE, CAMS_GEAR_GROUP, SPECIES_ESTIMATION_REGION) %>% dplyr::summarise(D = sum(CAMS_DISCARD), K = sum(SUBTRIP_KALL), Drate = mean(CAMS_DISCARD_RATE), Dmort = mean(DISC_MORT_RATIO)) %>% arrange(desc(D))%>% top_n(10) %>% dplyr::mutate(across(where(is.numeric), round, 4))%>% mutate(across(ends_with("D"), round, 0))%>%
		DT::datatable(caption = 'Discard totals for state trips by stock, gear and discard source for Goldentilefish')
```

```{r bluelinetilefish gear fed discard table }
db_bluelinetilefish %>% filter(FED_OR_STATE == 'FED') %>% group_by(DISCARD_SOURCE, CAMS_GEAR_GROUP, SPECIES_ESTIMATION_REGION) %>% dplyr::summarise(D = sum(CAMS_DISCARD), K = sum(SUBTRIP_KALL), Drate = mean(CAMS_DISCARD_RATE), Dmort = mean(DISC_MORT_RATIO)) %>% arrange(desc(D))%>% top_n(10) %>% dplyr::mutate(across(where(is.numeric), round, 4))%>% mutate(across(ends_with("D"), round, 0))%>%
		DT::datatable(caption = 'Discard totals for federal trips by stock, gear and discard source for Bluelinetilefish')
```

```{r bluelinetilefish gear state discard table }
db_bluelinetilefish %>% filter(FED_OR_STATE == 'STATE') %>% group_by(DISCARD_SOURCE, CAMS_GEAR_GROUP, SPECIES_ESTIMATION_REGION) %>% dplyr::summarise(D = sum(CAMS_DISCARD), K = sum(SUBTRIP_KALL), Drate = mean(CAMS_DISCARD_RATE), Dmort = mean(DISC_MORT_RATIO)) %>% arrange(desc(D))%>% top_n(10) %>% dplyr::mutate(across(where(is.numeric), round, 4 ))%>% mutate(across(ends_with("D"), round, 0))%>%
		DT::datatable(caption = 'Discard totals for state trips by stock, gear and discard source for Bluelinetilefish')
```
