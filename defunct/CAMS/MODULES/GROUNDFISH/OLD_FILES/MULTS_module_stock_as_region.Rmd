...
select STAT_AREA 
    , SPECIES_ESTIMATION_REGION as REGION
...
# } else{
  ...
  dplyr::select(SPECIES_ESTIMATION_REGION, STAT_AREA) %>% 
  dplyr::rename(AREA = STAT_AREA, STOCK_AREA = SPECIES_ESTIMATION_REGION)
...
CAMS_DISCARD_MORTALITY_STOCK = CAMS_DISCARD_MORTALITY_STOCK %>% 
  collect() %>% 
  mutate(STOCK_AREA = SPECIES_ESTIMATION_REGION) %>% 
...
ddat_focal <- c_o_dat2 %>% 
  ...
  relocate('COMMON_NAME','SPECIES_ITIS','NESPP3','STOCK_AREA', 'CAMS_GEAR_GROUP','DISC_MORT_RATIO') #'SPECIES_ESTIMATION_REGION',
...
#               dplyr::select(-NESPP3) %>% 
#               mutate(REGION = SPECIES_ESTIMATION_REGION)
