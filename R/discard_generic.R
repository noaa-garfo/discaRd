...
discard_generic <- function(con = con_maps
  ...
  foreach(
    ...
    all_dat = all_dat |>
      dplyr::mutate(OBS_DISCARD = case_when(SPECIES_ITIS == species_itis ~ DISCARD_PRORATE
                                            , TRUE ~ 0))
    ...
    STOCK_AREAS = tbl(con, sql('select * from CFG_STATAREA_STOCK')) |>
      dplyr::filter(ITIS_TSN == species_itis) |>
      dplyr::collect() |>
      dplyr::group_by(SPECIES_ESTIMATION_REGION, ITIS_TSN) |>
      dplyr::distinct(AREA) |>
    ...
    CAMS_DISCARD_MORTALITY_STOCK = tbl(con, sql("select * from CFG_DISCARD_MORTALITY_STOCK"))  |>
      dplyr::collect() |>
      dplyr::mutate(GEARCODE = CAMS_GEAR_GROUP
    ...
    STOCK_AREAS  = STOCK_AREAS |>
      dplyr::mutate(SPECIES_ESTIMATION_REGION = str_replace(SPECIES_ESTIMATION_REGION, '_', '-')) |>
      dplyr::mutate(SPECIES_ESTIMATION_REGION = str_replace(SPECIES_ESTIMATION_REGION, '_', '-'))
    ...
    ddat_focal <- all_dat |>
      ...
      dplyr::mutate(FY_TYPE = FY_TYPE
                    , FY = FY) |>
      dplyr::mutate(LIVE_POUNDS = SUBTRIP_KALL
                    ,SEADAYS = 0
      ) |>
      dplyr::left_join(y = STOCK_AREAS, by = 'AREA') |>
      dplyr::left_join(y = CAMS_GEAR_STRATA, by = 'GEARCODE') |>
    ...
    bdat_cy = ddat_focal |>
      ...
      dplyr::filter(SOURCE != 'ASM') |>
      dplyr::filter(substr(LINK1, 1,3) %!in% OBS_REMOVE$OBS_CODES)

    if(nrow(bdat_cy) > 0 ) {
      bdat_cy <- bdat_cy |>
        dplyr::mutate(DISCARD_PRORATE = DISCARD
                      , OBS_AREA = AREA
    ...
    if(exists("dest_strata_f")) {
      ...
      trans_rate_df <- trans_rate_df |>
        ...
        dplyr::mutate(n_obs_trips_p = dplyr::coalesce(n_obs_trips_p, 0)) |>
        dplyr::mutate(trans_rate = get_trans_rate(l_observed_trips = n_obs_trips_f
      ...
      trans_rate_df <- trans_rate_df |>
        ...
        dplyr::mutate(n_obs_trips_p = coalesce(n_obs_trips_p, 0)) |>
        dplyr::mutate(trans_rate = get_trans_rate(l_observed_trips = n_obs_trips_f
                                                  , l_assumed_rate = previous_season_rate
                                                  , l_inseason_rate = in_season_rate
        ),
    ...
    if(exists("d_focal")) {
      full_strata_table = trans_rate_df_full |>
        ...
        as_tibble() |>
        dplyr::mutate(SPECIES_ITIS_EVAL = species_itis
    ...
    if(nrow(bdat_cy > 0)) {
      bdat_2yrs = dplyr::bind_rows(bdat_prev_cy, bdat_cy)
    } else {
      bdat_2yrs = bdat_prev_cy
    }

    ddat_cy_2yr = dplyr::bind_rows(ddat_prev_cy, ddat_focal_cy)
    ddat_2yr = dplyr::bind_rows(ddat_prev, ddat_focal)
    ...
    BROAD_STOCK_RATE_TABLE = gear_only$allest$C |>
      ...
      dplyr::mutate(FY = as.numeric(sub("_.*", "", STRATA))
                    , FY_TYPE = FY_TYPE) |>
      dplyr::mutate(SPECIES_ESTIMATION_REGION = gsub("^([^_]+)_", "", STRATA)  |>
    ...
    joined_table = joined_table |>
      ...
      dplyr::mutate(COAL_RATE = dplyr::case_when(
        n_obs_trips_f >= 5 ~ final_rate  # this is an in season rate
        , n_obs_trips_f < 5 &
      ...
      dplyr::mutate(COAL_RATE = coalesce(COAL_RATE, BROAD_STOCK_RATE)) |>
      dplyr::mutate(SPECIES_ITIS_EVAL = species_itis
                    , COMNAME_EVAL = species$ITIS_NAME[i]
                    , FISHING_YEAR = FY
                    , FY_TYPE = FY_TYPE)
    ...
    joined_table = joined_table |>
      dplyr::mutate(STRATA_USED = case_when(
        DISCARD_SOURCE == 'O' & LINK3_OBS == 1 ~ ''
    ...
    joined_table = joined_table |>
      dplyr::mutate(DISC_MORT_RATIO = dplyr::coalesce(DISC_MORT_RATIO, 1)) |>
      dplyr::mutate(DISCARD = ifelse(DISCARD_SOURCE == 'O', DISC_MORT_RATIO*OBS_DISCARD # observed with at least one obs haul
    ...
    joined_table = joined_table |>
      dplyr::mutate(DISCARD_SOURCE = case_when(ESTIMATE_DISCARDS == 0 & DISCARD_SOURCE != 'O' ~ 'N'
                                               ,TRUE ~ DISCARD_SOURCE)) |>
      dplyr::mutate(DISCARD = case_when(ESTIMATE_DISCARDS == 0 & DISCARD_SOURCE != 'O' ~ 0.0
                                        ,TRUE ~ DISCARD))|>
      dplyr::mutate(CV = case_when(ESTIMATE_DISCARDS == 0 & DISCARD_SOURCE != 'O' ~ NA_real_
                                   ,TRUE ~ CV))
    ...
    joined_table <- joined_table |>
      ...
      make_strata_desc() |>
      get_covrow() |>
      dplyr::mutate(covrow = case_when(DISCARD_SOURCE =='N' ~ NA_real_
                                       , TRUE ~ covrow))
    ...
    joined_table = joined_table |>
      dplyr::mutate(SPECIES_ESTIMATION_REGION = str_replace(SPECIES_ESTIMATION_REGION, '-', '_')) |>
      dplyr::mutate(SPECIES_ESTIMATION_REGION = str_replace(SPECIES_ESTIMATION_REGION, '-', '_')) |>
      dplyr::mutate(STRATA_USED_DESC = str_replace(STRATA_USED_DESC, '-', '_')) |>
      dplyr::mutate(STRATA_USED = str_replace(STRATA_USED, '-', '_')) |>
      dplyr::mutate(STRATA_USED_DESC = str_replace(STRATA_USED_DESC, '-', '_')) |>
      dplyr::mutate(STRATA_ASSUMED = str_replace(STRATA_ASSUMED, '-', '_')) |>
      dplyr::mutate(FULL_STRATA = str_replace(FULL_STRATA, '-', '_'))
